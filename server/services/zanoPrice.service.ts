...
export async function backfill(fromMs: number, toMs: number) {
  ...
  while (currentStart < toMs) {
    ...
    await ZanoPrice.bulkCreate(candles, { updateOnDuplicate: ["price_close", "raw"] });

    console.log(`[BACKFILL] ${candles.length} candles saved, start: ${candles[0].ts_utc.toISOString()}`);

    currentStart = candles[candles.length - 1].ts_utc.getTime() + FOUR_HOURS_MS;
...
if (lastRow && lastRow.ts_utc) {
  const tmp = new Date(lastRow.ts_utc as any).getTime();
  lastTsMs = Math.min(tmp, nowAligned); 
}

const from = lastTsMs !== null
  ? lastTsMs + FOUR_HOURS_MS
  : nowAligned - 365 * 24 * 60 * 60 * 1000;
