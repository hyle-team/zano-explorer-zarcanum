...
const JSONPopup = (props: JSONPopupProps) => {
	const { popupState, setPopupState, json, bottomContent, hideJson } = props;
			...
			<div className={styles.json_popup__content}>
				{!hideJson && (
					<div className={styles.json_popup__content__json}>
						<JsonViewStyled data={json} />
					</div>
				)}
				<Button wrapper onClick={close} className={styles.json_popup__close_btn}>
					<CrossImg />
				</Button>
				{bottomContent}
			</div>
	...
	return popupState ? <Popup Content={PopupContent} settings={{}} close={onClose} blur /> : <></>;
};
