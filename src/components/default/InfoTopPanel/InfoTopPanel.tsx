...
function InfoTopPanel(props: InfoTopPanelProps) {
	...
	return (
		<div
			className={`${styles.blockchain__info__top} ${className} ${inputClosed ? styles.blockchain__input__closed : ''} ${burgerOpened ? styles.info__top__hidden : ''}`}
		>
			{back && (
			...
			<div className={styles.info__top__title}>
				...
				{!back && (
					<div
						className={`${styles.info__top__content} ${!contentNotHiding ? styles.hiding_element : ''}`}
					>
						{content || <></>}
					</div>
				)}
			...
			<div className={styles.info__top__input}>
				...
				<Button onClick={inputState ? onButtonClick : () => setInputClosed(!inputClosed)}>
					<SearchImg />
				</Button>
			</div>
		</div>
