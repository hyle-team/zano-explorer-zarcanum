...
function LatestBlocks({
	...
	const [pagesAmount, setPagesAmount] = useState(
		Math.ceil(((info?.height || 0) - 1) / parseInt(itemsOnPage || '0', 10)),
	);
	const [page, setPage] = useState(new BigNumber(latestBlocksInitState.page).toFixed());
	...
	useEffect(() => {
		async function fetchBlocks() {
			try {
				...
				const result = await Fetch.getBlockDetails(
					heightToRequest - items * pageNumber,
					items,
				);
				...
				const currentTxCount = transformed.reduce(
					(acc, block) => acc + (block.transactions || 0),
					0,
				);
				...
				if (currentTxCount > prevCount) {
					...
					setHeaderStatus(
						<>
							<span>
								{diff} more transaction{diff > 1 ? 's' : ''}
							</span>{' '}
							has come in
						</>,
					);
	...
	const tableElements = blocks.map((e, idx) => {
		const { hash } = e;
		...
		return [
			...
			<AliasText key={idx} href={hashLink}>
				{hash}
			</AliasText>,
		];
	});
	...
	useEffect(() => {
		function formatLastUpdated() {
			setLastUpdatedText(
				lastUpdated ? Utils.timeElapsedString(lastUpdated / 1000, true) : undefined,
			);
		}
			...
			<h3 className={styles.blockchain__latest_blocks__title}>
				...
				{lastUpdated && (
					<span className={styles.status__badge}>
						<InfoIcon /> Last updated {lastUpdatedText}
					</span>
				)}
			</h3>
