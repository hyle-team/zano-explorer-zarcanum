...
function StatsPanel(props: {
	...
	const infoHeight =
		Utils.formatNumber(Math.min(info?.height || 0, info?.database_height || 0), 0) || '...';
	...
	const hashrate =
		Utils.toShiftedNumber(
			info?.current_network_hashrate_350,
			state.netMode === 'TEST' ? 0 : 9,
			2,
		) || '...';
		...
		return (
			<div className={styles.main__top__item}>
				...
				<div className={styles.item__value}>
					{isAmountString ? <p>{amount + (!customCurrency ? ' ZANO' : '')}</p> : amount}
					{percent && (
	...
	function BottomItem(props: {
		...
		return (
			<div className={styles.main__bottom__item} style={style}>
				...
				<div className={styles.bottom__item__value}>{children}</div>
			</div>
		);
	}
	...
	function InfoTop() {
		return (
			<div className={styles.info__main__top}>
				<TopItem title="Staked Coins (est)" amount={stackedCoins} percent={percentage} />
				<TopItem title="Dev Fund" amount={devFund} />
				<TopItem title="Real Time APY" amount={`${APY}%`} customCurrency={true} />
				<TopItem
					title="Zano Burned"
					amount={
