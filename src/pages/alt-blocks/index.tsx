...
function AltBlocks() {
	...
	useEffect(() => {
		async function fetchAltBlocks() {
			...
			const result = await Fetch.getAltBlocksInfo(
			...
			setAltBlocks(Utils.transformToBlocks(result, false, true));
		}
	...
	const tableHeaders = [
		'HEIGHT',
		'TIMESTAMP (UTC)',
		'ACTIAL TIMESTAMP (UTC)',
		'SIZE',
		'TRANSACTIONS',
		'HASH',
	];

	const tableElements = altBlocks.map((e, idx) => {
		const { hash } = e;
	...
	return (
		<div className={styles.alt_blocks}>
			...
			/>
			<InfoTopPanel burgerOpened={burgerOpened} title="Alt-blocks" />
