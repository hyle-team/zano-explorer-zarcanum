...
function Block(props: BlockProps) {
	...
	const [blockInfo, setBlockInfo] = useState<BlockInfo | null>(
		props.blockData?.blockInfo || null,
	);
	const [transactions, setTransactions] = useState<Transaction[]>(
		props.blockData?.transactionsDetails || [],
	);
					...
					<table>
						<tbody>
								...
								<td>
									{blockInfo?.timestamp
										? Utils.formatTimestampUTC(
												+new Date(parseInt(blockInfo?.timestamp, 10)),
											)
										: '-'}
								</td>
							</tr>
							<tr>
								...
								<td>
									<Link href="/">
										{Utils.shortenAddress(blockInfo?.tx_id ?? '-')}
									</Link>
								</td>
							</tr>
							<tr>
								...
								<td>
									{blockInfo?.actualTimestamp
										? Utils.formatTimestampUTC(
												parseInt(blockInfo?.actualTimestamp.toString(), 10),
											)
										: '-'}
								</td>
							</tr>
							...
							<tr>
								...
								<td>
									{blockInfo?.major_version ?? '-'} /{' '}
									{blockInfo?.minor_version ?? '-'}
								</td>
							</tr>
						</tbody>
					</table>
					<table>
						<tbody>
							...
							<tr>
								<td>Previous ID:</td>
								<td>
									<Link href={`/block/${blockInfo?.prev_id}`}>
										{Utils.shortenAddress(blockInfo?.prev_id ?? '-')}
									</Link>
								</td>
							</tr>
							<tr>
	...
	return (
		<div className={styles.block}>
			...
			<StatsPanel
				...
				fetchedInfo={props.info}
			/>
			<BlockInfo />
			<div className={styles.block__transactions}>
				<h2>Transactions</h2>
				<Table headers={tableHeaders} elements={tableElements} />
			</div>
		</div>
	);
}
