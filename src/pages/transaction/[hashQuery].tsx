...
function Transaction({ visibilityInfo, info, transactionsData }: TransactionPageProps) {
	const [burgerOpened, setBurgerOpened] = useState(false);

	const [transactionInfo, setTransactionInfo] = useState<TransactionInfo | null>(
		transactionsData?.transactionInfo || null,
	);
	const [blockOrigin, setBlockOrigin] = useState<Block | null>(
		transactionsData?.blockOrigin || null,
	);
	...
	const insRows = transactionInfo
		? transactionInfo.ins.map((e) => [
				...
				e.globalIndexes.length > 1 ? (
					<Link href="/" onClick={(event) => showIndexesClick(event, e)}>
						Show all...
					</Link>
				) : (
					<Link
	...
	const outsRows = transactionInfo
		? transactionInfo.outs.map((e, idx) => [
				e.amount,
				<div key={idx} className={styles.transaction__outs__keys}>
					{e.publicKeys.map((e, idx) => (
						<p key={idx}>{e}</p>
					))}
				</div>,
				e.globalIndex,
	...
	function GlobalIndexesPopup({ close }: { close: () => void }) {
		...
		return (
			<div className={styles.transaction__indexes__popup}>
				<h3>Input ring set ({popupIndexes.length})</h3>
				<div>
					{popupIndexes.map((e) => (
						<Link href="/" key={e} onClick={(event) => onIndexClick(event, amount, e)}>
							{e}
						</Link>
					))}
				</div>
				<Button wrapper className={styles.popup__cross} onClick={close}>
					<CrossImg />
				</Button>
			</div>
		);
			...
			<div className={styles.transaction__info}>
				...
				<table>
					<tbody>
						...
						<tr
							className={
								styles.transaction__confirmation +
								(!transactionInfo?.confirmations
									? ` ${styles.transaction__unconfirmed}`
									: '')
							}
						>
							<td>Confirmations</td>
				...
				<Table
					...
					elements={[
						[
							...
							blockOrigin?.height || '',
							blockOrigin?.timestamp
								? Utils.formatTimestampUTC(parseInt(blockOrigin.timestamp, 10))
								: '',
						],
					]}
