...
function Aliases(props: AliasesPageProps) {
	...
	const fetchMatrixAliases = useCallback(async () => {
		...
		const aliases = result.addresses.map(
			(e: { address: string; hasMatrixConnection: boolean }[]) => {
		...
		async function fetchAliasesCount() {
			...
			setAliasStats({
				...
				premiumAliasesAmount:
					typeof premiumAliasesAmount === 'number' ? premiumAliasesAmount : undefined,
			});
		}
			...
			<a href={link}>
				...
				>
					...
					<>
						<ConnectionImg />
						{hovered && (
							<div className={styles.connection_icon__tooltip}>
								<p>Matrix Connection</p>
								<div className={styles.connection_icon__tooltip__arrow}></div>
							</div>
						)}
					</>
				</div>
	...
	const tableElements = aliases.map((e, idx) => [
		<Alias key={idx} alias={e.alias} hasMatrixConnection={e.hasMatrixConnection} />,
		e.address,
	]);
	...
	return (
		<div className={styles.aliases}>
			<Header page="Aliases" burgerOpened={burgerOpened} setBurgerOpened={setBurgerOpened} />
			<InfoTopPanel
